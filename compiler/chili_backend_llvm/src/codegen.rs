...
impl<'w, 'cg, 'ctx> Codegen<'cg, 'ctx> {
    ...
    pub(super) fn gen_binding_module_unpack_pattern(
        &mut self,
        state: &mut CodegenState<'ctx>,
    ...
    pub(super) fn gen_binding_struct_unpack_pattern(
        &mut self,
        state: &mut CodegenState<'ctx>,
        pattern: &UnpackPattern,
        ty: &TyKind,
    ...
    pub(super) fn gen_binding_tuple_unpack_pattern(
        &mut self,
        state: &mut CodegenState<'ctx>,
        pattern: &UnpackPattern,
        ty: &TyKind,
    ...
    fn gen_cast_inner(
        ...
        match (from_ty, target_ty) {
            (TyKind::Bool, TyKind::Int(_)) | (TyKind::Bool, TyKind::Uint(_)) => self
                ...
                .into(),
            (TyKind::Int(_) | TyKind::Uint(_), TyKind::Int(_) | TyKind::Uint(_)) => self
                .builder
            ...
            // pointer <=> int | uint
            (TyKind::Pointer(..), TyKind::Int(..) | TyKind::Uint(..)) => self
                .builder
                .build_ptr_to_int(
                    value.into_pointer_value(),
                    cast_type.into_int_type(),
            ...
            // int | uint <=> pointer
            (TyKind::Int(..) | TyKind::Uint(..), TyKind::Pointer(..)) => self
                .builder
                .build_int_to_ptr(
                    value.into_int_value(),
                    cast_type.into_pointer_type(),
