...
impl SyntaxError {
    pub fn unknown_builtin_function(
        span: &Span,
        builtin: String,
    ) -> Diagnostic<usize> {
        Diagnostic::error()
    ...
    pub fn uninit_var(name: String, span: &Span) -> Diagnostic<usize> {
        Diagnostic::error()
            ...
            .with_labels(vec![Label::primary(span.file_id, span.range.clone())
                .with_message("not initialized")])
    }

    pub fn struct_field_specified_more_than_once(
        span: &Span,
        name: String,
    ) -> Diagnostic<usize> {
        Diagnostic::error()
    ...
    pub fn duplicate_symbol(
        ...
        Diagnostic::error()
            ...
            .with_labels(vec![
                Label::primary(
                    duplicate_span.file_id,
                    duplicate_span.range.clone(),
                )
                .with_message("duplicate definition here"),
                Label::secondary(
                    already_defined_span.file_id,
                    already_defined_span.range.clone(),
                )
                .with_message(format!(
    ...
    pub fn duplicate_struct_field(
        ...
        Diagnostic::error()
            ...
            .with_labels(vec![
                Label::primary(symbol_span.file_id, symbol_span.range.clone())
                    .with_message("field defined more than once"),
                Label::secondary(
                    defined_symbol_span.file_id,
                    defined_symbol_span.range.clone(),
                )
                .with_message(format!(
                    "previous definition of `{}` here",
                    field_name
...
impl TypeError {
    pub fn negative_array_len(span: &Span, len: isize) -> Diagnostic<usize> {
        Diagnostic::error()
            .with_message(format!(
                "array length cannot be negative, found {}",
                len
            ))
            .with_labels(vec![Label::primary(span.file_id, span.range.clone())])
    }
    ...
    pub fn struct_destructor_on_invalid_type(
        ...
        Diagnostic::error()
            .with_message(format!(
                "cannot use struct destructor on type `{}`",
                ty
            ))
            .with_labels(vec![Label::primary(span.file_id, span.range.clone())])
    }
    ...
    pub fn tuple_destructor_on_invalid_type(
        ...
        Diagnostic::error()
            .with_message(format!(
                "cannot use tuple destructor on type `{}`",
                ty
            ))
            .with_labels(vec![Label::primary(span.file_id, span.range.clone())])
    }
    ...
    pub fn duplicate_destructor_field(
        span: &Span,
        field: Ustr,
    ) -> Diagnostic<usize> {
        Diagnostic::error()
    ...
    pub fn cant_solve_inference(span: &Span) -> Diagnostic<usize> {
        Diagnostic::error()
            ...
            .with_labels(vec![
                Label::primary(span.file_id, span.range.clone()).with_message("can't infer type")
            ])
    }
