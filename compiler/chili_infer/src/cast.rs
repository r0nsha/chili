...
pub fn ty_can_be_casted(from: &TyKind, to: &TyKind) -> bool {
    from == to
        || match (from, to) {
            // bool <=> int
            ...
            (
                TyKind::Pointer(t1, from_mutable),
                TyKind::MultiPointer(t2, to_mutable),
            )
            | (
                TyKind::MultiPointer(t1, to_mutable),
                TyKind::Pointer(t2, from_mutable),
            ) if t1 == t2 && can_coerce_mut(*from_mutable, *to_mutable) => true,

            // pointer(array) => multi-pointer
            (
                TyKind::Pointer(t, from_mutable),
                TyKind::MultiPointer(t_ptr, to_mutable),
