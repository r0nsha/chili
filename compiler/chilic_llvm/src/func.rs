...
impl<'cg, 'ctx> Codegen<'cg, 'ctx> {
                    ...
                    _ => {
                        match entity.value.as_ref() {
                            Some(expr) => match &expr.kind {
                                // * if the entity is a fn or fn-type, don't
    ...
    pub(super) fn gen_fn(
            ...
            params.iter().zip(func.proto.params.iter()).enumerate()
        {
            ...
            self.gen_entity_pattern_from_value(
                &mut state,
                &param.pattern,
                param_ty,
                value,
    ...
    pub(super) fn gen_fn_call_expr(
        ...
        // println!("{}", call.callee.display_name_and_entity_span().value);
