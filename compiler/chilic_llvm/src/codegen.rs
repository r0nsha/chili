...
impl<'cg, 'ctx> Codegen<'cg, 'ctx> {
    pub fn codegen(&mut self) {
        ...
        self.gen_top_level_entity(root_module.info, startup_entity);
    }

    pub(super) fn find_or_gen_entity_by_name(
        &mut self,
    ...
    pub(super) fn find_or_gen_top_level_decl(
        ...
        if let Some(entity) = module.find_entity(symbol) {
            if !entity.should_codegen {
                unreachable!()
            }

            self.gen_top_level_entity(module_info, entity)
        } else if let Some(use_) = module.find_use(symbol) {
            if use_.use_path.is_empty() {
                let decl = CodegenDecl::Module(use_.module_info);

                self.get_or_insert_new_module(module_info.name)
                    .insert(use_.alias, decl);

                decl
            } else {
                self.resolve_decl_from_use(use_)
            }
        } else {
            unreachable!(
    ...
    pub(crate) fn resolve_decl_from_use(
        ...
        if !use_.use_path.is_empty() {
            ...
            let mut current_module_info = use_.module_info;

            for symbol in use_.use_path.iter() {
    ...
    pub(super) fn gen_top_level_entity(
        &mut self,
        module_info: ModuleInfo,
        entity: &Entity,
    ) -> CodegenDecl<'ctx> {
        match entity.value.as_ref() {
            Some(expr) => match &expr.kind {
                ExprKind::Fn(func) => {
        ...
        let global_value = if entity.lib_name.is_some() {
    ...
    pub(super) fn gen_entity_pattern_from_expr(
        &mut self,
    ...
    pub(super) fn gen_entity_pattern_from_value(
        &mut self,
        state: &mut CodegenState<'ctx>,
        pattern: &Pattern,
        ty: &Ty,
