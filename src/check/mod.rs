...
impl Check for ast::Ast {
    fn check(
        ...
        let res = match self {
                ...
                match &kind.maybe_deref_once() {
                            ...
                            Ok(index) => match elements.get(index) {
                                Some(field_ty) => {
                                    ...
                                    Ok(Res::new_maybe_const(
                                        sess.tycx.bound(field_ty.clone(), access.span),
                                        const_value,
                                    ))
                                }
                    ...
                    ty @ Type::Struct(st) => match st.find_field(access.member) {
                        Some(field) => {
                            ...
                            Ok(Res::new_maybe_const(
                                sess.tycx.bound(field.ty.clone(), access.span),
                                const_value,
                            ))
                        }
                ...
                if is_const_tuple {
                    ...
                    if is_tuple_type {
                        ...
                        *self = ast::Ast::ConstValue(ast::Const {
                        ...
                        Ok(Res::new_const(
                            sess.tycx.bound(kind.create_type(), lit.span),
                            const_value,
                        ))
                    } else {
            ...
            ast::Ast::ConstValue(const_value) => panic!(),
